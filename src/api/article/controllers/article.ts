...
export default createCoreController('api::article.article', ({ strapi }) => ({
  async full(ctx) {
    try {
      ...
      const normalized = Array.isArray(entries) 
        ? entries.map((entry: any) => normalizeEntity(entry, baseUrl))
        : [normalizeEntity(entries, baseUrl)];

      return ctx.send({ data: normalized });
    } catch (err) {
      strapi.log.error('article.full error', err);
  ...
  async related(ctx) {
    try {
      ...
      const article = currentArticle as any;
      const categoryId = article.news_category?.id || article.news_category?.documentId;
      const tagIds = Array.isArray(article.tags) 
        ? article.tags.map((tag: any) => tag.id || tag.documentId).filter(Boolean)
        : [];

      // Build filters for related articles
      const filters: any = {
        documentId: {
          $ne: documentId, // Exclude current article
        },
      };
      ...
      let relatedArticles = await strapi.documents('api::article.article').findMany({ 
        filters,
        populate,
        status: 'published',
        sort: { createdAt: 'desc' },
      ...
      if (relatedArticles && relatedArticles.length > 0) {
        relatedArticles = relatedArticles
          .filter((a: any) => a.documentId !== documentId)
          .slice(0, 3);
      }

      // If we don't have enough articles from the same category, get more recent articles
      if (!relatedArticles || relatedArticles.length < 3) {
        const additionalArticles = await strapi.documents('api::article.article').findMany({ 
          filters: {
            documentId: {
              $ne: documentId,
            },
        ...
        // Exclude already fetched articles
        const fetchedIds = relatedArticles ? relatedArticles.map((a: any) => a.documentId) : [];
        const filtered = additionalArticles
          ? additionalArticles.filter((a: any) => !fetchedIds.includes(a.documentId))
          : [];

        relatedArticles = [
          ...(relatedArticles || []),
          ...filtered.slice(0, 3 - (relatedArticles?.length || 0)),
        ];
      }
      ...
      // Normalize all entries
      const normalized = relatedArticles.map((entry: any) => normalizeEntity(entry, baseUrl));

      return ctx.send({ data: normalized });
    } catch (err) {
      strapi.log.error('article.related error', err);
      return ctx.throw(500, err?.message || 'Internal Server Error');
    }
  },
}));

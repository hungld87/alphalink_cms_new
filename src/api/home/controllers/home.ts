...
export default createCoreController('api::home.home', ({ strapi }) => ({
  async full(ctx) {
    try {
      ...
      const entry = await strapi.documents('api::home.home').findFirst({ 
        populate,
        status: 'published'
      } as any);
      ...
      strapi.log.error('home.full error', err);

...
export default createCoreController('api::q-and-a.q-and-a', ({ strapi }) => ({
  async full(ctx) {
    try {
      ...
      // Fetch q-and-a (single type)
      const entry = await strapi.documents('api::q-and-a.q-and-a').findFirst({ 
        populate,
        status: 'published'
      } as any);
      ...
      strapi.log.error('q-and-a.full error', err);
